...
LOG_LEVEL = "INFO"
logger = logging.getLogger("localwhatyousee")
...
MODEL_PATH = os.getenv(
    "LOCALWHATYOUESEE_MODEL",
    "mlx-community/SmolVLM2-500M-Video-Instruct-mlx",
)
MAX_TOKENS = int(os.getenv("LOCALWHATYOUESEE_MAX_TOKENS", "64"))
TEMPERATURE = float(os.getenv("LOCALWHATYOUESEE_TEMPERATURE", "0.7"))
TOP_K = int(os.getenv("LOCALWHATYOUESEE_TOP_K", "64"))
TOP_P = float(os.getenv("LOCALWHATYOUESEE_TOP_P", "0.95"))
MAX_SIDE = int(os.getenv("LOCALWHATYOUESEE_MAX_SIDE", "0"))
PROMPT = (
...
def _init_model():
    global model, processor, config, prompt, _initialized
    if _initialized:
        return
    config = load_config(MODEL_PATH)
    model_type = config.get("model_type")
    is_jina = model_type == "jvlm"
    model, processor = load(MODEL_PATH, trust_remote_code=not is_jina)
    prompt = apply_chat_template(processor, config, PROMPT, num_images=1)
    _initialized = True
    logger.info("Loaded model %s", MODEL_PATH)


def _resize_pil(image: Image.Image, max_side: int) -> Image.Image:
    if max_side <= 0:
        return image
    w, h = image.size
...
def _run_generate(image: Image.Image):
    ...
    if TOP_P:
        kwargs["top_p"] = TOP_P
    try:
        return generate(model, processor, prompt, [image], **kwargs)
    except TypeError:
        return generate(model, processor, prompt, [image], max_tokens=MAX_TOKENS, temperature=TEMPERATURE)
...
app = FastAPI(title="LocalWhatYouSee", lifespan=lifespan)
...
def _analyze_local(image_url: str) -> dict:
    start = time.time()
    image = _decode_image(image_url)
    image = _resize_pil(image, MAX_SIDE)
    try:
        response = _run_generate(image)
        ...
        logger.error("Local model request failed: %s", exc)
        return {
            "label": "ERROR",
            "distracted": False,
            "reason": "error",
...
def analyze(req: ImageRequest):
    ...
    try:
        result = _analyze_local(req.image)
        ...
        <div class="panel side-panel">
          ...
          <div class="controls">
            ...
            <div class="pipeline">
              ...
              <span id="pipeline">webcam -> mlx</span>
            </div>
          </div>
        </div>
      </section>
